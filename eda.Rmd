...
6. **pow_garazu**: Powierzchnia garażu.
...
Najważniejsze anomalie widać w dolnym prawym rogu wykresu: są to nieruchomości, które mimo bardzo dużego metrażu mają relatywnie niskie ceny. Odstają od rynkowego trendu, co może wskazywać na ich nietypowość. 
...
3. Dzielnice Landmark i Green Hills, zawierające za małą liczbę obserwacji.
```{r}
...
1. Segment budżetowy: od Meadow Village do Mitchell, najliczniejsza grupa dzielnic, obejmująca zakres cenowy od poniżej 100 000 USD do trochę powyżej 150 000 USD. 
Najtańsza i jednocześnie jedyna dzielnica z medianą ceny poniżej 100 000 USD to Meadow Village. Wzrost cen między kolejnymi dzielnicami w tej grupie jest łagodny,  jednak na końcu grupy następuje pierwszy wyraźny próg cenowy oddzielający ją od segmentu średniego.
...
4. Segment premium: od Northridge do Stone Brooks, obserwujemy najbardziej drastyczny skok cenowy na całym wykresie. Różnica mediany między Veenker, a Northridge wynosi ponad 50 000 USD. Wszystkie dzielnice w tej grupie przekraczają barierę 300 000 USD, co wyraźnie izoluje je od reszty miasta jako elitarne enklawy o zupełnie innym standardzie lub prestiżu.
...
## Struktura zabudowy -- Czy poszczególne dzielnice znacząco różnią się strukturą zabudowy?
...
2. Segment średni
    ...
    B. Anomalią jest dzielnica Crawford, której rozkład wiekowy jest zbliżony do obszarów z segmentu budżetowego przy jednoczesnym braku nowych inwestycji. Mimo wieku nieruchomości utrzymują ceny segmentu średniego, co sugeruje ich lepszy stan techniczny, większy metraż lub unikalny charakter architektoniczny.
  
    C. Bloomington Heights stanowi przykład nowszego budownictwa szeregowego, które standardem i ceną odróżnia się od starszych odpowiedników z lat 70.
...
3. Segment drogi
    ...
    B. Veenker to obszar domów jednorodzinnych z lat 70. i 80., który mimo dawnego roku budowy, utrzymuje wysoką wartość rynkową, co sugeruje istnienie innych unikalnych cech podnoszących wartość tych nieruchomości.
 
4. Segment premium

    A. Northridge Heights i Stone Brook to najnowsze dzielnice w zestawieniu, co potwierdza, że w najwyższym przedziale cenowym kluczową determinantą wartości jest nowoczesność. W Stone Brook luksusowe szeregówki stanowią niemal połowę oferty.
...
1. W segmencie budżetowym znajdują się nieruchomości o bardzo zróżnicowanej powierzchni:
    ...
    A. Dzielnica Meadow Village charakteryzuje się najniższą medianą powierzchni w całym mieście oraz niską zmiennością. Potwierdza to jej charakter jako osiedla taniej zabudowy szeregowej o powtarzalnym standardzie.
...
2. W segmencie średnim w większości dzielnic (np. Gilbert, Sawyer West, Northwest Ames) nieruchomości są do siebie zbliżone metrażem. Mediana powierzchni w tych dzielnicach znajduje się w przedziale 140 - 160 m^2^, co może sugerować standard rynkowy.
    ...
    B. Dzielnica Greens, która składa się z domów szeregowych wybudowanych w bardzo wąskim zakresie czasu, charakteryzuje się nieruchomościami o bardzo podobnym metrażu.
  
3. W segmencie drogim budynki nie charakteryzują się znacznie większym metrażem od segmentu średniego: metraż nie jest tu kluczowym determinantem ceny - jest nim tu wiek budynku
...
4. W segmencie premium występuje wyraźny skok w metrażu, ale struktura dzielnic wykazuje pewne różnice:
    ...
    B. Dzielnice Northridge Heights i Stone Brook prezentują bardzo podobny rozkład powierzchni. Obie charakteryzują się szerokim zakresem dostępnych metraży, co wyjaśnia fakt, że znajdują się tu zarówno domy szeregowe jak i domy jednorodzinne.
...
```
Analiza rozkładu powierzchni garażowej w podziale na segmenty cenowe ujawnia wyraźne różnice standardów rynkowych w poszczególnych segmentach:
...
szeregowe %>%
  ...
  theme(
    axis.text.x = element_text(angle = 0, hjust = 0.5, face = "bold", size = 9),
    axis.text.y = element_text(face = "bold", size = 9),
...
## Rozkład powierzchni działki w zabudowie szeregowej
...
Wykres gęstości powierzchni działek przy zabudowie szeregowej ujawnia ogromne różnice w strukturze gruntów przynależnych do nieruchomości w poszczególnych segmentach cenowych:
...
2 . Rozkład dla segmentu średniego jest znacznie szerszy. Jest tu istotna grupa nieruchomości z działkami w przedziale 300–400 m^2^. Oznacza to, że w tej klasie cenowej powierzchnia gruntu jest nadal istotnym czynnikiem wartości. W tym segmencie występują też nieruchomości o znacząco większych działkach. 
...
1. Rynek w Ames charakteryzuje się prawostronną asymetrią cenową. Analiza wykazała silną, dodatnią zależność między powierzchnią mieszkalną a ceną sprzedaży. Relacja ta nie jest jednorodna w całym zbiorze, wraz ze wzrostem metrażu rośnie również rozrzut cenowy. Ceny największych nieruchomości są znacznie bardziej zróżnicowane i trudniejsze do przewidzenia w oparciu o samą wielkość. W ich wycenie – obok metrażu – kluczową rolę odgrywają inne zmienne, takie jak lokalizacja w konkretnej dzielnicy czy jakość wykończenia.

2. Analiza median cen pozwoliła na wyodrębnienie czterech wyraźnych segmentów rynkowych, które różnią się nie tylko ceną, ale i profilem zabudowy:

    - Segment Budżetowy: Zdominowany przez najstarsze domy (lata 20.–40.) oraz szeregówki z lat 70. Charakteryzuje się dużym zróżnicowaniem metrażu, ale najniższym standardem wykończenia oraz niewielkim garażem lub jego brakiem.
    ...
    - Segment Premium: Grupa najdroższych nieruchomości, wyraźnie oddzielona cenowo od reszty rynku. Wyróżnia się skokowym wzrostem powierzchni mieszkalnej, powszechnym standardem garaży trzystanowiskowych oraz dominacją najwyższych ocen jakości.
    
3. Szczegółowa analiza domów szeregowych wykazała, że ten typ zabudowy pełni na rynku dwie odmienne funkcje:
